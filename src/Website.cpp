...
void YaSolR::WebsiteClass::updateCards() {
  ...
  if (freq == 0 && Mycila::Dimmer::isZCDEnabled()) {
    freq = 1000000.0f / 2 / Mycila::Dimmer::getZCSemiPeriod();
  }
  ...
  _status(_output1Dimmer, KEY_ENABLE_OUTPUT1_DIMMER, dimmerO1.isDimmerEnabled(), Mycila::Dimmer::isZCDEnabled(), YASOLR_LBL_179);
  ...
  _status(_output2Dimmer, KEY_ENABLE_OUTPUT2_DIMMER, dimmerO2.isDimmerEnabled(), Mycila::Dimmer::isZCDEnabled(), YASOLR_LBL_179);
  ...
  _status(_zcd, KEY_ENABLE_ZCD, true, Mycila::Dimmer::isZCDEnabled(), YASOLR_LBL_110);
#endif
}
